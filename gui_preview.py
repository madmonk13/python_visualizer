...
class PreviewPanel:
    ...
    def _get_status_color(self, status_type):
        """Get status-specific color that works in both modes"""
        colors = {
            'default': self._get_text_color(),
            'blue': '#4A90E2',      # Lighter blue that works in both modes
            'green': '#5CB85C',     # Lighter green
            'red': '#D9534F',       # Lighter red
            'yellow': '#F0AD4E'     # Orange/yellow
        }
        return colors.get(status_type, colors['default'])
    ...
    def _create_panel(self):
        ...
        # Info label with mode-aware text color
    ...
    def set_info(self, message, color="default"):
        """Update the info label with mode-aware colors"""
        text_color = self._get_status_color(color)
        self.info_label.config(text=message, foreground=text_color)
